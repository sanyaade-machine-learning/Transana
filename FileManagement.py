...
# TODO:  Figure out access to SRB from Non-Windows platforms.
      ...
      if targetLbl.GetLabel() == _('Local:'):
          if targetDir != '':
             ...
             for file in filenames:
                self.FileManagementWindow.SetStatusText(_('Copying %s to %s') % (file, targetDir))
...
class FileManagement(wx.Dialog):
   ...
   def srbDisconnect(self):
      ...
      srb.srbDaiDisconnect(self.srbConnectionID.value)
   ...
   def OnMouseOver(self, event):
      ...
      if self.activeSide == 'Source':
         ...
         # The path differs for local and remote file systems
         if self.lblSource.GetLabel() == _('Local:'):
      ...
      if event.GetId() == ID_BTNCOPY:
         ...
         self.SetStatusText(_('Copy %s files from %s to %s') % (tempText, sourceDir, targetDir))
      ...
      # Move button
         ...
         if sourceFile.GetSelectedItemCount() == 0:
            ...
            tempText = _('selected')
         # Specify source and destination Folder/Collections
         self.SetStatusText(_('Move %s files from %s to %s') % (tempText, sourceDir, targetDir))

      # Delete button
      elif event.GetId() == ID_BTNDELETE:
         self.SetStatusText(_('Delete selected files from %s') % sourceDir)

      # New Folder button
      elif event.GetId() == ID_BTNNEWFOLDER:
         self.SetStatusText(_('Create new folder in %s') % sourceDir)
   ...
   def OnCopyMove(self, event):
      ...
      if (sourceLbl == _('Local:')) and \
         ...
         for file in fileList:
            ...
            if moveFlag:
               self.SetStatusText(_('Moving %s to %s') % (fileName, targetDir))
            ...
            # ... otherwise we want to copy the file
               ...
               self.SetStatusText(_('Copying %s to %s') % (fileName, targetDir))
      ...
      # Copy or Move from SRB Collection to Local Drive
         ...
         for file in fileList:
            ...
            # Find the File Size
            buf = ' ' * 25
            srb.srbDaiMcatGetData(2, sourceFile.FindItem(-1, file), buf, 25)
      ...
      # Copy or Move from SRB Collection to SRB Collection
         ...
         for file in fileList:
            ...
            Result = srb.srbDaiObjToCollCopy(  
                        self.srbConnectionID,    # Connection ID
                        0,                       # Catalog Type - Use 0
                        file,                    # Source File name
                        0,                       # "replNum" - Use 0. Transana does not support replications
                        sourceDir,               # Source Collection
                        'unknown',               # File Type - "unknown" works
                        0,                       # Size - ??
                        targetDir,               # Destination Collection
                        self.srbResource,        # Resource (could be different than old Resource!)  Transana does not support alternate Resource Specification at this time!
...
#         dlg = wx.MessageDialog(self, "SRB to SRB Copy/Move has not yet been implemented.", style = wx.OK | wx.ICON_EXCLAMATION)
#         dlg.ShowModal()
#         dlg.Destroy()
   ...
   def DeleteFile(self, lbl, path, filename):
      ...
      if lbl == _('Local:'):
         ...
         self.SetStatusText(_('Deleting %s') % fileName)
         # Delete the file
         os.remove(fileName)
      else:
         fileName = path + '/' + filename
         self.SetStatusText(_('Deleting %s') % fileName)
         delResult = srb.srbDaiRemoveObj(self.srbConnectionID, filename, path, 0)
         if delResult < 0:
   ...
   def OnDelete(self, event):
      ...
      if sourceFile.GetItemCount() > 0:
         ...
         self.SetStatusText(_('Removing %s') % sourcePath)
         if sourceLbl.GetLabel() == _('Local:'):
            # Get the Parent Item
            ...
            RemoveResult = srb.srbDaiRemoveCollection(self.srbConnectionID, 0, sourcePath);
            if RemoveResult < 0:
               self.srbErrorMessage(RemoveResult)
            if sourceLbl.GetLabel() == otherLbl.GetLabel():
                self.SetStatusText(_('Reading Collections from SRB.'))
                # Recreate the appropriate Collection Name, which has gotten lost somewhere along the way
                if self.SRBConnDlg.rbSRBStorageSpace.GetSelection() == 1:
                   srbCollection = self.SRBConnDlg.editCollectionRoot.GetValue() + self.SRBConnDlg.editOtherUserName.GetValue() + '.' + self.SRBConnDlg.editOtherDomain.GetValue()
                else:
                   srbCollection = self.SRBConnDlg.editCollectionRoot.GetValue() + self.SRBConnDlg.editUserName.GetValue() + '.' + self.SRBConnDlg.editDomain.GetValue()
   ...
   def OnUpdateDB(self, event):
       ...
       while 1:
           ...
           fileList.append(control.GetItemText(item))
       ...
       if not DBInterface.UpdateDBFilenames(self, filePath, fileList):
           infodlg = Dialogs.InfoDialog(self, _('Update Failed.  Some records that would be affected are locked by another user.'))
           infodlg.ShowModal()
           infodlg.Destroy()
   ...
   def srbAddChildNode(self, srbDir, treeNode, srbCollectionName):
      ...
      if "__WXMSW__" in wx.PlatformInfo:
          ...
          srb = ctypes.cdll.LoadLibrary("srbClient.dylib")
      srbCatalogCount = srb.srbDaiMcatQuerySubColls(self.srbConnectionID.value, 0, srbCollectionName)
      ...
      if srbCatalogCount < 0:
         ...
         while srbCatalogCount > 0:
            ...
            for loop in range(srbCatalogCount):
               ...
               srb.srbDaiMcatGetColl(loop, buf, len(buf))
            ...
            srbCatalogCount = srb.srbDaiMcatGetMoreColls(self.srbConnectionID.value)
            # Check to see if Sub-Collections are returned or if an error code (negative result) is generated
            if srbCatalogCount < 0:
               self.srbErrorMessage(srbCatalogCount)
   ...
   def OnConnect(self, event):
      ...
      if sourceLbl.GetLabel() == _('Remote:'):
         # Change the label
         sourceLbl.SetLabel(_('Local:'))
         ...
         if (self.lblSource.GetLabel() == _('Local:')) and \
            (self.lblDest.GetLabel() == _('Local:')):
            self.srbDisconnect()
      # If the label is not "Remote", we should connect to the SRB
         ...
         if self.srbConnectionID == None:
            ...
            if self.SRBConnDlg.ShowModal() == wx.ID_OK:
               ...
               connResult = SRBConnectFx(ctypes.byref(self.srbConnectionID),
                                         srbCollection,
                                         self.SRBConnDlg.editDomain.GetValue(),
                                         self.SRBConnDlg.editSRBResource.GetValue(),
                                         self.SRBConnDlg.editSRBSEAOption.GetValue(),
                                         self.SRBConnDlg.editSRBPort.GetValue(),
                                         self.SRBConnDlg.editSRBHost.GetValue(),
                                         self.SRBConnDlg.editUserName.GetValue(),
                                         self.SRBConnDlg.editPassword.GetValue())
               ...
               self.srbResource = self.SRBConnDlg.editSRBResource.GetValue()
         ...
         if connResult >= 0:
            ...
            # Add a Root Item to the SRB Dir Tree
            srbRootItem = sourceSrbDir.AddRoot(srbCollection)
            # Initiate the recursive call to get all Collections from the SRB and add them to the SRB Dir Tree
            self.srbAddChildNode(sourceSrbDir, srbRootItem, srbCollection)
            # Expand the Root Item to show all the first-level collections
   ...
   def OnNewFolder(self, event):
      ...
      if self.activeSide == 'Source':
         ...
         if otherLbl == _('Local:'):
            ...
            other = self.srbSourceDir
      self.SetStatusText(_('Create new folder for %s') % path)
      ...
      newFolder = wx.TextEntryDialog(self, _('Create new folder for %s:') % path, _('Create new folder'), style = wx.OK | wx.CANCEL | wx.CENTRE)
      ...
      if newFolder.ShowModal() == wx.ID_OK:
         if sourceLbl == _('Local:'):
            ...
            NewCollection = srb.srbDaiNewCollection(self.srbConnectionID, 0, path, newFolder.GetValue());
   ...
   def OnRefresh(self, event):
      ...
      if self.sourceDir.GetPath() != '':
         ...
         self.RefreshFileList(self.lblSource.GetLabel(), self.sourceDir.GetPath(), self.sourceFile, self.sourceFilter)
      if self.destDir.GetPath() != '':
         # if a path is defined on the right-hand side, refresh the right hand side controls
         self.RefreshFileList(self.lblDest.GetLabel(), self.destDir.GetPath(), self.destFile, self.destFilter)
   ...
   def GetFullPath(self, control):
      ...
      pathList = control.GetItemText(item)
      ...
      while item != control.GetRootItem():
         ...
         pathList = control.GetItemText(item) + '/' + pathList  # '/', not os.sep, because the SRB ALWAYS uses unix-style paths.
   ...
   def IsEmpty(self, lbl, path):
       ...
       if lbl == _('Local:'):
          ...
          srbCatalogCount = srb.srbDaiMcatQueryChildDataset(self.srbConnectionID.value, 0, path)
   ...
   def OnDirSelect(self, event):
      ...
      self.SetStatusText(_('Directory changed to %s') % path)
         ...
         if srbCatalogCount < 0:
            ...
            while srbCatalogCount > 0:
               ...
               for loop in range(srbCatalogCount):
                  ...
                  srb.srbDaiMcatGetData(0, loop, buf, len(buf))
               ...
               srbCatalogCount = srb.srbDaiMcatGetMoreData(self.srbConnectionID.value)
   ...
   def itemsToShow(self, path, filelist, selection, isSrbList):
       ...
       for item in filelist:
           if isSrbList: 
               ...
               if (selection == _('All files (*.*)')) | (srch.count(string.lower(os.path.splitext(item)[1])) > 0):
   ...
   def srbErrorMessage(self, errorCode):
      ...
      dlg = Dialogs.ErrorDialog(self, _('An error has occurred in working with the Storage Resource Broker.\nError Code: %s\nError Message: "%s"') % (errorCode, errorMsg))
      dlg.ShowModal()
      dlg.Destroy()
