            ...
            try:
                ...
                if (len(data) > 1) and (data[:data.find(' ')] == 'C'):
                    ...
                    if int(version) < VERSION:
                        self.connection.send('M MessageServer: The Transana Message Server you have connected to is newer than your copy ||| ')
                        self.connection.send('M MessageServer: of Transana-MU.  Please upgrade your copy of Transana-MU immediately. ||| ')
                    ...
                    # Detect new Transana versions...
                        ...
                        self.connection.send('M MessageServer: The Transana Message Server you have connected to is older than your copy ||| ')
                        self.connection.send('M MessageServer: of Transana-MU.  Please ask your system administrator to upgrade your copy ||| ')
                        self.connection.send('M MessageServer: of the Transana-MU Message Server immediately. ||| ')
                        self.connection.send('M MessageServer: - ||| ')
                        self.connection.send('M MessageServer: Please do not proceed.  Data corruption could result. ||| ')
                        self.connection.send('M MessageServer: - ||| ')
                    else:
...
if RUNASWINSERVICE:
    ...
    class TransanaMessageService(win32serviceutil.ServiceFramework):
        ...
        _svc_display_name_ = "Transana Message Server, v. 2.30"
